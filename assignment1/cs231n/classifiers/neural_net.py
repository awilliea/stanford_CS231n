...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        loss = -np.sum(np.log(correct_score/sum_score))
        ...
        #############################################################################
        dS = scores/(sum_score.reshape((-1,1)))
        dS[range(N),y] -= 1
        dW2 = hidden.T.dot(dS)
        db2 = np.ones(N).dot(dS)
        dhidden = dS.dot(W2.T)
        drelu = (hidden > 0) * dhidden
#         dhidden[hidden < 0 ] = 0
#         drelu = dhidden
        dW1 = X.T.dot(drelu)
        db1 = np.ones(N).dot(drelu)
        
        grads['b2'] = db2/N
        grads['b1'] = db1/N
        grads['W1'] = dW1/N + 2*W1*reg
        grads['W2'] = dW2/N + 2*W2*reg
    ...
    def predict(self, X):
        ...
        ###########################################################################
        hidden = np.maximum((X.dot(self.params['W1'])+self.params['b1']),0)
